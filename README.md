...
```
GitHub 上的插件           你的服务器                访客的浏览器
┌─────────────┐    同步     ┌──────────────────┐    访问    ┌──────────┐
│ extension.zip │ ──────▶ │ 原样存为 zip         │ ──────▶ │ 插件列表 │
│ release page │             │ （开发者模式直接加载）│           │ 一键安装 │
└─────────────┘             └──────────────────┘            └──────────┘
                                      ▲
                                      │ 你也可以手动上传 .zip
```
...
| 痛点 | 解法 |
|---|---|
| GitHub release 下載繁琐 | 后台配置仓库，自动同步拉取最新 zip |
| 插件更新靠人记 | 有“同步”按钮，点一下就是最新版 |
| 没统一的浏览体验 | 内置插件商店页面，有列表、搜索、详情页、安装引导 |
| 手动打包 `.crx` 麻烦 | 不需要打包，直接存 zip，开发者模式加载即可 |
...
- **跨浏览器** — 同时提供 Chrome（开发者模式加载）和 Firefox 的安装链接（底层都是同一份 zip）
...
带 Hero Banner、热门分类、为您推荐，夜晚暗色主题。

### 详情页 — 单个插件介绍

![详情页](docs/images/detail.png)

图标 / 下载按钮 / 项目地址 / 评分 / 安装说明一应俱全。

### 开发者后台 — 从 GitHub 一键导入

![GitHub 导入](docs/images/import.png)

填一个 GitHub 仓库地址，自动拉取 release 打包。

### 开发者后台 — 我的插件列表

![我的插件](docs/images/my-plugins.png)

所有插件在一个页面统一管理，查看版本 / 安装量 / 评分。
...
└── packages/
    └── <plugin-slug>/
        └── extension.zip    # 原始 zip（同时作为 crx / xpi 提供下载）
        └── icon.svg         # 插件图标
```
